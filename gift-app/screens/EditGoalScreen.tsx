...
import {
  formatDateUntilDay,
  formatDateUntilDayFromDateObject,
} from "../utils/file";
...
const EditGoalScreen = ({ route, navigation }) => {
  ...
  const settingDateChange = (event, selectedDate) => {
    if (selectedDate) {
      const currentDate = formatDateUntilDayFromDateObject(
        selectedDate || date
      );
    ...
    <>
      <View style={styles.input_wrapper}>
        <View style={styles.input_body}>
          <Text style={[styles.input_body_text, { marginTop: 10 }]}>
            大きな夢
          </Text>
          ...
          <Text
            style={[styles.input_body_text, { marginTop: windowHeight * 0.1 }]}
          >
            目標
          </Text>
          <View style={styles.action}>
